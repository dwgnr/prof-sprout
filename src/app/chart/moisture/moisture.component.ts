...
export class MoistureComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.dataService.getSoilMoisture(this.startTime, this.endTime, this.resolution, this.hours)
            .subscribe((result) => this.moistureData = result.data,
    ...
    createChart() {
        ...
        for (const datum of this.moistureData) {
    ...
    reload() {
        ...
        this.dataService.getSoilMoisture(this.startTime, this.endTime, this.resolution, this.hours)
            .subscribe((result) => this.moistureData = result,
            (error) => this.handleError(error),
            () => this.createChart()
        );
    ...
    onChartChange(stepSize, displayDateFormat, resolution, hours) {
        ...
        this.hours = hours;
        this.dataService.getSoilMoisture(this.startTime, this.endTime, this.resolution, this.hours)
            .subscribe((r) => this.moistureData = r.data,
                (error) => this.handleError(error),
                () => this.createChart()
            );
    }
